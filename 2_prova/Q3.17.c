...
void Print_vector(double local_vector[], int n, int local_n, int my_rank, MPI_Datatype type, MPI_Comm comm);
// void Read_vector(local_vector, local_n, n, my_rank, type);
// void Print_vector(local_vector, local_n, n, my_rank, type);
...
#  endif
  //  Read_vector("x", local_x, n, local_n, my_rank, comm);
   Read_vector(local_x, n, local_n, my_rank, type, comm);
#  ifdef DEBUG
  //  Print_vector("x", local_x, n, local_n, my_rank, comm);
   Print_vector(local_x, n, local_n, my_rank, type, comm);
#  endif
...
int my_rank, MPI_Datatype type, MPI_Comm  comm){
  ...
  MPI_Scatter(vector, 1, type, local_vector,1,type,0,comm);
  free(vector);
}
// void Read_vector(
//       char      prompt[]     /* in  */, 
...
void Print_vector(double local_vector[], int n, int local_n, int my_rank, MPI_Datatype type, MPI_Comm comm){
  ...
  if(my_Rank == 0){
    vector = malloc(n*sizeof(double));
    MPI_Gather(local_vector,1,tyoe,vector,1,type, 0,comm);
    ...
    MPI_Gather(local_vector,1,type,vector,type,0,comm);
  }
}
// void Print_vector(
//       char      title[]     /* in */, 
